...
WORK_ROOT="${TMPDIR:-/tmp}/xdepends_mac"
...
find_app_bundle() {
    ...
    for candidate in \
        "${BUILD_DIR}/src/gui/xdepends.app" \
        "${BUILD_DIR}/src/gui/Release/xdepends.app" \
        "${BUILD_DIR}/src/gui/XDepends.app" \
        "${BUILD_DIR}/src/gui/Release/XDepends.app"
    do
        if [[ -d "${candidate}" ]]; then
    ...
    find "${BUILD_DIR}" -maxdepth 6 -type d \( -name "xdepends.app" -o -name "XDepends.app" \) -print -quit
}
